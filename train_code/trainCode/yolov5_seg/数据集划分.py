...
base_path = './xm_ylg/imgandtxt'  # 转化完的图片和txt所在的文件夹
dataset_path = './xm_ylg/result'  # 新建一个文件夹
val_size, test_size = 0.1, 0.1  # 这里把test设置为0，也就是train：val = 9:1
# val_size = 0.1
...
for i in train_id:
    print(i)
    shutil.copy(f'{base_path}/{i.split("_")[0]}.{postfix}', f'{dataset_path}/images/train/{i}.{postfix}')
    print (f'{base_path}/{i.split("_")[0]}.{postfix}')
    shutil.copy(f'{base_path}/{i}.txt', f'{dataset_path}/labels/train/{i}.txt')

for i in val_id:
    shutil.copy(f'{base_path}/{i.split("_")[0]}.{postfix}', f'{dataset_path}/images/val/{i}.{postfix}')
    shutil.copy(f'{base_path}/{i}.txt', f'{dataset_path}/labels/val/{i}.txt')

for i in test_id:
    shutil.copy(f'{base_path}/{i.split("_")[0]}.{postfix}', f'{dataset_path}/images/test/{i}.{postfix}')
